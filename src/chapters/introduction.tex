...
Stars have a rich history of being used as astrophysical laboratories to search for dark matter. Depending on the type of dark matter being searched for, there are various signals one can look for.
Light bosonic dark matter, such as ALPs and dark photons, can be produced in the plasma of stars, altering the energy transport properties within them.
...
There are three regimes in which we are interested in solving Eq.~\ref{ch1:eq:DM_num_Sun}. The simplest case is when evaporation and annihilation are both negligible, then the solution is simply, 
\begin{equation}
    \NDM(t) = C t,
\end{equation}
...
For example, one might locate a NS with radio telescopes such as the Square-Kilometer-Array (SKA) and determine their age through their spin-down rate. 
