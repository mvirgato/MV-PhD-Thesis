...
There are three regimes we are interested in solving this equation for. The simplest case is if both evaporation and annihilation are negligible, or at least have characteristic timescales longer than the age of the Sun. Then the solution is just, 
\begin{equation}
    \NDM(t) = C t,
\end{equation}
indicating that the dark matter will simply continue to grow over time.

Next, assume that annihilations are negligible  ($A = 0$) and that the capture and evaporation rates are constant in time. The result is
\begin{equation}
...
\end{equation} 
where $\alpha$ is the fraction of evaporated dark matter. 
