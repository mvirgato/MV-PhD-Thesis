...
In this section, we focus on addressing the latter concern, relegating the discussion on the NS opacity in \fixMV{Appendix~\ref{app:NS_ocacity}} as it is not required for the remainder of this work. 
...
In deriving Eq.~\ref{ch3:eq:capturefinal} we had assumed that the DM velocity at infinity, $u_\chi$, can be neglected, such that any interaction where the DM loses energy resulted in its capture. If we instead keep the leading order $u_\chi$ contribution to the total DM energy, the DM energy at infinity is instead
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:Echir}
\end{equation}
Therefore, the amount of energy that must be lost in order for the DM to be captured is 
\begin{align}
    E_\chi^C(r) & =  \frac{1}{2}u_\chi^2 \frac{m_\chi}{\sqrt{B(r)}}. \\
                & \sim 6\times 10^{-7}\GeV \left(\frac{u_\chi}{270\km\s^-1}\right)^2 \left(\frac{m_\chi}{1\GeV}\right)\left(\frac{0.5}{B(r)}\right)^{1/2},
\end{align} 

For DM masses $m_\chi\gtrsim 10^6\GeV$ that scatters on nucleon targets, the capture probability is smaller than $1$ and becomes tiny as we increase the DM mass to large values. To account for this effect, we proceed in the following way. First, instead of setting the DM speed at infinity to $0$, as we did in the previous section, we now assume that the DM particles have a speed $u_\chi\ll 1$ that follows a Maxwell-Boltzmann (MB) distribution, Eq.~\ref{eq:DMveldist}. 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=.45\textwidth]{capture_1/probs_n0.pdf}
    \includegraphics[width=.45\textwidth]{capture_1/probs_n1.pdf}\\
    \includegraphics[width=.45\textwidth]{capture_1/probs_n2.pdf}
...
We  define the probability of a DM particle to be captured after exactly $N$ scatterings by averaging over the MB energy distribution   
\begin{eqnarray}
...
\begin{figure}
    \centering
    \includegraphics[width=.45\textwidth]{capture_1/mstar_B_n0.pdf}
    \includegraphics[width=.45\textwidth]{capture_1/mstar_B_n1.pdf}    
    \includegraphics[width=.45\textwidth]{capture_1/mstar_B_n2.pdf}      
