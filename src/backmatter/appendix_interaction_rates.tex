...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Derivation of Dark Matter Interaction Rates in Degenerate Media}
...
The most general form of the interaction rate, following Ref.~\cite{Bertoni:2013bsa_dec_DarkMatterThermalization}, can be written as
\begin{align}
...
The integration range for $\Ei$ is obtained from the two $\Theta$-functions. There are two cases to be considered, $t<0$ and $t>0$. In the former case, the range become $\Ei^{t^-}<\Ei<\infty$ and for the latter $0<\Ei<\Ei^{t^+}$. These integration bounds are obtained from Eq.~\ref{eq:cosn}, by settting $\cos\theta = 1$, and are given by
\begin{align}
...
% \label{sec:inelastic_scatter_full}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% \begin{equation}
%     \frac{(2\mi q_0 + t)^2}{4t} +(2\mi +q_0)\kinFi +\kinFi^2 < 0\label{eq:tpp1}
% \end{equation}
...
% % \;&\;+ \mathcal{I}_{n,m}\left(f_2^{(m,n)}(t),t_E^+,t_{\mu^+}^+,t_E^-,t_{\mu^-}^+\right)\Theta(\kinFi-q_0)  \nonumber\\
% % \;&\;+\mathcal{I}_{n,m}\left(f_2^{(m,n)}(t),t_E^+,t_{\mu^-}^-,t_E^-,t_{\mu^+}^-\right)\Theta(\kinFi-q_0)  \nonumber\\
% % \;&\;\left. + \mathcal{I}_{n,m}\left(f_2^{(m,n)}(t),t_E^+,t_{\mu^+}^+,t_E^-,t_{\mu^+}^-\right)\Theta(q_0-\kinFi) \right], \label{eq:gammafull}
% % \end{align}
...
% \begin{align}
%     \tth^{0,0} & = \frac{49}{4\sqrt{2}}\frac{\mu}{(1 +\mu)^2}\frac{\sqrt{\mi \kinFi^3}}{n_n \sigma_{surf} \Teq^2}\\
%     & \sim 1.427\times 10^8\yrs\frac{\mu}{(1 +\mu)^2}\left(\frac{\kinFi(0)}{0.2\GeV}\right)^{3/2}\left(\frac{2\times10^{-45}\cm^2}{\sigma_{surf}}\right)\left(\frac{0.375\fm^{-3}}{n_n(0)}\right)\left(\frac{10^3\K}{\Teq}\right)^2\\
%     \tth^{1,0} & =  \frac{729}{76\sqrt{2}}\frac{\mu^2}{(1 +\mu)^2(1+\mu^2)}\frac{1 - B(\Rstar)}{B(\Rstar)}\frac{(\mi \kinFi)^{3/2}}{n_n \sigma_{surf} \Teq^3}\\
%     & \sim 1.217\times 10^{18}\yrs\frac{\mu^2}{(1 +\mu)^2(1+\mu^2)}\left(\frac{\kinFi(0)}{0.2\GeV}\right)^{3/2}\left(\frac{2\times10^{-45}\cm^2}{\sigma_{surf}}\right)\left(\frac{0.375\fm^{-3}}{n_n(0)}\right)\left(\frac{10^3\K}{\Teq}\right)^3\\
%     \tth^{2,0} & =  \frac{9150625}{1231312\sqrt{2}}\frac{\mu^3}{(1 +\mu)^2(1+\mu^2)^2}\left(\frac{1 - B(\Rstar)}{B(\Rstar)}\right)^2 \frac{\sqrt{\mi^5 \kinFi^3}}{n_n \sigma_{surf} \Teq^4}\\
%     & \sim 1.027\times 10^{28}\yrs\frac{\mu^3}{(1 +\mu)^2(1 + \mu^2)^2}\left(\frac{\kinFi(0)}{0.2\GeV}\right)^{3/2}\left(\frac{2\times10^{-45}\cm^2}{\sigma_{surf}}\right)\left(\frac{0.375\fm^{-3}}{n_n(0)}\right)\left(\frac{10^3\K}{\Teq}\right)^4
% \end{align}
...
%     \frac{1}{\tth}  & = \left.\frac{a_0}{\tth^{0,0}}\right|_{\alpha_{0,0}=1} + \left.\frac{a_1}{\tth^{1,0}}\right|_{\alpha_{1,0}=1} + \left.\frac{a_2}{\tth^{2,0}}\right|_{\alpha_{2,0}=1} \label{eq:reicptherm}
% \end{align}
